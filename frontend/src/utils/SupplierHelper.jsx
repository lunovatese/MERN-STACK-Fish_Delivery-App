...
export const SupplierButtons = ({ Id, onSupplierDelete }) => {
    ...
    return (
        <div className="flex space-x-3">
            <button className="px-3 py-1 bg-teal-600 text-white" onClick={() => navigate(`/admin-dashboard/supplier/view-supplier/${Id}`)}> View</button>
            <button className="px-5 py-1 bg-teal-600 text-white" onClick={() => navigate(`/admin-dashboard/supplier/edit-supplier/${Id}`)}> Edit</button>
            <button className="px-5 py-1 bg-red-600 text-white" onClick={handleDelete}>Delete</button>
        </div>
    );
};
